...
class TestGeneticAlgorithmResults(unittest.TestCase):
    ...
    def test_get_contributions(self):
        ...
        #         i_trace: trace.Trace,
        #         config: ga_configs.VoltageOptimizationConfig) -> List[
        #     pd.DataFrame]:
        i_trace = trace.Trace(
            t=[i for i in range(6)],
            y=[5 for _ in range(6)])
        current_response = trace.CurrentResponseInfo(protocol=None)
        current_response.currents.append(
            [trace.Current(name='I_Na', value=1.0),
             trace.Current(name='I_K1', value=2.0)])
        current_response.currents.append(
            [trace.Current(name='I_Na', value=3.0),
             trace.Current(name='I_K1', value=15.0)])
        current_response.currents.append(
            [trace.Current(name='I_Na', value=15.0),
             trace.Current(name='I_K1', value=3.0)])
        current_response.currents.append(
            [trace.Current(name='I_Na', value=1.0),
             trace.Current(name='I_K1', value=0.0)])
        current_response.currents.append(
            [trace.Current(name='I_Na', value=16.0),
             trace.Current(name='I_K1', value=13.0)])
        current_response.currents.append(
            [trace.Current(name='I_Na', value=2.0),
             trace.Current(name='I_K1', value=3.0)])
        i_trace.current_response_info = current_response
        ...
        contributions = genetic_algorithm_results.get_contributions(
            i_trace=i_trace,
            config=config)
        print(contributions)
